...
- **Delivery Manager:** prepares the validation assignment and four-project matrix, verifies deployment prerequisites, and routes any exception.
- **Implementation Engineer:** opens clean sessions on each required surface, prepares missing-capability/dependency simulations, runs technical regressions, and records evidence.
...
1. The Implementation Engineer gives Tony a one-page deployment table naming every required project/surface, deployed artifact version, and hash.
...
If the supported environment uses PowerShell startup regression scripts, the Implementation Engineer also runs the approved repository command and records it verbatim. Tony checks only that the summary contains zero failures or errors.

### Step 3 — Perform a clean ordinary start on every required surface

For each row of the four-project matrix:

1. The Implementation Engineer opens a new clean chat/session with no inherited EENOS mission or mode.
...
7. The Implementation Engineer saves the entire transcript and marks that surface PASS or FAIL.
...
3. Ask to begin a mission for which the Implementation Engineer has made one required test dependency unavailable in the sandbox. The system must block before handoff and name what is missing.
4. Repeat with an optional dependency unavailable. The system may continue only if the limitation is visibly disclosed.

The Implementation Engineer, not Tony, creates the safe missing-dependency conditions.
...
3. The Implementation Engineer simulates loss of a required capability. Tony verifies that work stops or follows an explicitly approved degraded path; it must not continue silently.
4. The Implementation Engineer simulates a failed re-entry. Tony verifies that the result explains whether the old valid session can resume or must remain suspended.

### Step 7 — Record the decision

The Implementation Engineer completes one AC-001 through AC-018 sheet per required surface plus one combined matrix. RM-010 cannot pass unless every required surface passes and `DEF-RM-010-001` has no unresolved blocking finding.
