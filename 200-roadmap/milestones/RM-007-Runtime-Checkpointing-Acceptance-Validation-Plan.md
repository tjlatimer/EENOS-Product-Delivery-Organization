...
- **Delivery Manager:** converts this plan into the authorized validation assignment, confirms access and evidence locations, and coordinates exception routing.
- **Implementation Engineer:** prepares the disposable test folder, runs every command, creates damaged or competing test records, captures screenshots and logs, and explains results in ordinary language.
- **Independent Reviewer:** checks the evidence after the Implementation Engineer finishes. The reviewer does not repair failures.
...
1. The Implementation Engineer gives Tony a folder named `RM-007-Validation-Run-<date>`.
...
3. The Implementation Engineer shows the frozen software commit and writes it at the top of the result sheet.
...
The Implementation Engineer opens PowerShell at the software-repository root and runs:
...
2. The Implementation Engineer uses the approved checkpoint workflow to finish that sample session and create a checkpoint in the disposable folder.
3. The Implementation Engineer opens the resulting checkpoint record read-only and points out:
...
1. The Implementation Engineer copies the normal checkpoint; the original is never edited.
2. In the copy only, the Implementation Engineer changes the saved mission text after the checkpoint was created.
3. The Implementation Engineer runs the approved validation/read-back operation against the altered copy.
...
1. The Implementation Engineer runs a prepared automated test that sends two requests using the same request key.
2. The result must show one checkpoint outcome, not two independently committed duplicates.
3. The Implementation Engineer then runs a prepared automated test that releases two requests from the same parent at the same time.
...
1. The Implementation Engineer creates a parent checkpoint and a child checkpoint.
2. The Implementation Engineer attempts, in the disposable store only, to remove or supersede the parent while the child still depends on it.
3. Tony verifies that the system blocks the unsafe action or preserves enough lineage for recovery.
4. The Implementation Engineer displays the RM-008 handoff record for the eligible checkpoint.
...
1. The Implementation Engineer completes AC-001 through AC-014 using the evidence collected above and any additional automated cases.
