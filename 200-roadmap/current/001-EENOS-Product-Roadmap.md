...
| RM-013 | EENOS Governance Framework | Medium | Major | Governance | Define the governance hierarchy among Constitution, governance documents, ADRs, processes, standards, runtime specifications, and implementation artifacts so future modules know where decisions belong. | Validation | 2026-07-15 | Architecture-to-implementation pipeline review | Tony | v1.2 | `repository/100-governance/001-Governance-Framework.md` (GOV-001); `repository/100-governance/workflow/001-Architecture-Decision-Record-Process.md` (PROC-001); `repository/000-standards/templates/001-ADR-Template.md` (TMPL-ADR-001); `software-repository/docs/rm013-eenos-governance-framework.md`; `software-repository/tools/governance_audit.py` | Modernized scope delivered: establishes 7-Tier Precedence Hierarchy (CONST > GOV > ADR > STD > PROC > SPEC > Implementation), Dual-Domain Governance separation (Product vs. Creative Canon), ADR process and template, taxonomy updates in STD-001, and automated metadata compliance auditor. In Validation pending Tony's review. |
...
| RM-027 | Optional Multidimensional Scene Development Prompts | Unprioritized | Minor | Story Development / Scene Development Record | Add seven optional free-form prompts to the Scene Development Record so writers can consider a scene's story function, plot progression, character progression, reader emotional journey, living-world detail, thematic reinforcement, and concrete scene outcome without turning those dimensions into a mandatory checklist. | Approved | 2026-08-04 | Delivery Manager handoff requesting optional Scene Development Record fields | Tony | Future | Tony approval recorded 2026-08-04; RM-001; RM-016; `98_Workflow/AI_Runtime/STD_Scene_Development_Record_v1.0.md` | Approved for Delivery Lead handoff without changing the current roadmap sequence. Place the prompts in a dedicated optional section and preserve the supplied helper-text meaning. Every field shall support multiline text and may remain blank; the entire section may be skipped or collapsed where supported. No minimum completion, validation, scoring, completeness indicator, approval gate, export gate, or other workflow rule may depend on these fields. Communicate that scenes need not contribute equally to every dimension. Keep Story Function (authorial or structural purpose within the novel) separate from Scene Outcome (the concrete story change by scene end). |
...
- **Priority:** Unprioritized; approved without displacing established release priorities.
- **Current status:** Approved; ready for Delivery Lead interpretation.
- **Objective:** Add seven optional, free-form scene-development prompts without creating completion, approval, scoring, validation, or export gates.
- **Why this comes next:** RM-025 acceptance reconciliation is complete, and RM-027 is the approved item already carrying a complete Planning Handoff Package.
- **Scope boundary:** Preserve every prompt as optional and keep Story Function distinct from Scene Outcome; implementation design remains with delivery roles.
- **Material alternative:** RM-022 and RM-015 remain the next-release provider-neutral capability pair if Tony chooses release sequencing over the already-prepared RM-027 handoff.
